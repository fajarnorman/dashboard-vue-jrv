<template>
    <div class="content-wrapper">
        <div id="customer-services">
            <page-header titleName="List Nomor CS" :isCreate="true"></page-header>
            <div class="content">
                ...
                <el-table
					...
					<el-table-column label="Number" prop="phone_number"></el-table-column>
                                ...
                                <div class="item-forms-details">
                                    <div class="forms-details">
                                        <el-row :gutter="20">
											...
											<el-col :span="12">
                                                <el-form-item label="Number" prop="phone_number">
                                                    <el-input type="number" v-model="forms.phone_number">
                                                        <template v-if="forms.type == 'whatsapp'" slot="prepend">62</template>
                                                    </el-input>
                                                </el-form-item>
                                            </el-col>
											<el-col :span="24">
                                                <el-form-item label="Tipe" prop="tipe">
...
export default {
    ...
    data(){
        return{
            ...
            rules: {
				...
				phone_number: [ { required: true, message: "Inputan Harus diisi", trigger: "blur" } ],
				type: [ { required: true, message: "Inputan Harus diisi", trigger: "change" } ],
            },
            dataTextCreate: "Buat Nomor CS",
            dataTextUpdate: "Edit Nomor CS",
            confirmDescDelete: "Anda yakin akan menghapus data Nomor CS dari daftar?",
            confirmTitleDelete: "Hapus Nomor CS",
            textDataCreateSuccess: "Data Nomor CS Berhasil ditambahkan.",
            textDataUpdateSuccess: "Data Nomor CS Berhasil diupdate.",
            textDataDeleteSuccess: "1 Items Berhasil dihapus.",
        }
    },
    created(){
        this.$FUNCTIONS.checkPermissions(this.$dataRole['.cms.customerservice.get'])
    },
    mounted(){
        this.getPost()
    ...
    methods:{
        ...
        handleCurrentChange(val) {
			if(this.$dataRole['.cms.customerservice.*.get']){
				if (val) {
        ...
        submitSave(formName, formtype) {
            ...
            if(this.forms.type == 'whatsapp'){
                let dataNumber = this.forms.phone_number
                if(dataNumber.charAt(0) == '0'){
                    dataNumberType = '62' + dataNumber.substr(1)
                }else{
                    if(dataNumber.substring(0, 2) == '62'){
                        dataNumberType = dataNumber
                    }else{
                        dataNumberType = '62' + dataNumber
                    }
                }
            }else{
                dataNumberType = this.forms.phone_number
            }

            dataValue = {
                name: this.forms.name,
                phone_number: dataNumberType,
