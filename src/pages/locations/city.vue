<template>
    <div class="content-wrapper">
        <div id="locations-city">
            <page-header titleName="Kota" :isCreate="true"></page-header>
            <div class="content">
                                        ...
                                        <el-row :gutter="20">
											<el-col :span="24">
                                                <el-form-item label="Provinsi" prop="province_id">
													<el-select filterable v-model="forms.province_id">
														<el-option
															v-for="(data, index) in itemsDataProvince"
															:key="index"
															:value="data.id"
...
export default {
    ...
    data(){
        return{
            ...
            dataTextCreate: "Buat Kota",
            dataTextUpdate: "Edit Kota",
            confirmDescDelete: "Anda yakin akan menghapus data Kota dari daftar?",
            confirmTitleDelete: "Hapus Kota",
            textDataCreateSuccess: "Data Kota Berhasil ditambahkan.",
            textDataUpdateSuccess: "Data Kota Berhasil diupdate.",
    ...
    methods:{
		...
		async getPostProvince(){
			try{
				let url = this.$baseUrl + this.sectionApiProvince + "?limit=50&page=1"
				this.loadingProvince = true
				const response = await this.$services.getList(url)
				this.loadingProvince = false
				if(response.status == 200){
                    this.itemsDataProvince = response.data.data
                }else{
                    this.$FUNCTIONS.checkErrorResponse(response.status)
                }
			}catch(err){
        ...
        handleCurrentChange(val) {
            if (val) {
                // if(this.$dataRole['.cms.location.province.*.get']){
					...
					this.forms.province_id = this.itemDataDetail.province_id
