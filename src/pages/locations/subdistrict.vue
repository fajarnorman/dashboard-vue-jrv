<template>
    <div class="content-wrapper">
        <div id="locations-subdistrict">
            <page-header titleName="Kecamatan" :isCreate="true"></page-header>
            <div class="content">
											...
											<el-col :span="24">
                                                <el-form-item label="Kota" prop="city_id">
													<el-select
														...
														<el-option
															v-for="(data, index) in itemsDataCity"
															:key="index"
															:value="data.id"
...
export default {
    name: 'locations-subdistrict',
    components: { pageHeader, loadingSvg },
    data(){
        return{
			sectionApi: "location/subdistrict",
            ...
            rules: {
				...
				city_id: [ { required: true, message: "Inputan Harus diisi", trigger: "change" } ],
            },
            dataTextCreate: "Buat Kecamatan",
            dataTextUpdate: "Edit Kecamatan",
            confirmDescDelete: "Anda yakin akan menghapus data Kecamatan dari daftar?",
            confirmTitleDelete: "Hapus Kecamatan",
            textDataCreateSuccess: "Data Kecamatan Berhasil ditambahkan.",
            textDataUpdateSuccess: "Data Kecamatan Berhasil diupdate.",
    ...
    methods:{
		...
		async getPostCity(search){
            try{
				let url = this.$baseUrl + this.sectionApiCity + "?limit=50&page=1&search=" + search
                this.loadingCity = true
                const response = await this.$services.getList(url)
				this.loadingCity = false
                if(response.status == 200){
                    this.itemsDataCity = response.data.data
                }else{
                    this.$FUNCTIONS.checkErrorResponse(response.status)
                }
            }catch(err){
		...
		async getPostSync(){
            try{
                ...
                this.loading = false
                if(response.status == 200){
					this.messagesuccess = "Berhasil Sinkronkan Kota"
					this.getPost()
                }else{
                    this.$FUNCTIONS.checkErrorResponse(response.status)
                }
        ...
        handleCurrentChange(val) {
            if (val) {
                // if(this.$dataRole['.cms.location.province.*.get']){
					...
					if(this.itemDataDetail.city){
						this.getPostCity(this.itemDataDetail.city.name)
						setTimeout(() => {
							this.loadingForm = false
							this.forms.city_id = this.itemDataDetail.city_id
						}, 500)
					}
