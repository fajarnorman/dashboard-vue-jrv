<template>
    <div class="content-wrapper">
        <div id="permissions">
            <page-header titleName="Permission List" :isCreate="true"></page-header>
            <div class="content">
                                        ...
                                        <el-row :gutter="20">
                                            ...
                                            <el-col :span="24">
                                                <el-form-item label="Permissions">
                                                    ...
                                                    <el-checkbox-group v-model="forms.permissions" class="attribute-check" v-if="itemsDataPermissionAuth.length !== 0">
                                                        <el-checkbox-button
                                                            v-for="dataRoles in itemsDataPermissionAuth"
                                                            :label="dataRoles"
                                                            :key="dataRoles">{{dataRoles}}</el-checkbox-button>
                                                    </el-checkbox-group>
													<el-checkbox-group v-model="forms.permissions" class="attribute-check" v-else>
                                                        <el-checkbox-button
                                                            v-for="dataRoles in itemsDataPermission"
                                                            :label="dataRoles"
                                                            :key="dataRoles">{{dataRoles}}</el-checkbox-button>
                                                    </el-checkbox-group>
                                                </el-form-item>
...
export default {
    ...
    data(){
        return{
            sectionApi: "roles",
            sectionApiListPermission: "permissionList",
            limit: 10,
            page: 1,
            loading: false,
            loadingForm: false,
            loadingPermission: false,
            itemDataPermissionCards: [],
            itemsData: [],
			itemsDataPermission: [],
			itemsDataPermissionAuth: [],
            itemDataDetail: [],
            totalPage: 0,
            searchQuery: "",
            titleForms: "",
            ...
            dataTextCreate: "Buat Permission",
            dataTextUpdate: "Edit Permission",
            confirmDescDelete: "Anda yakin akan menghapus data Permission dari daftar?",
            confirmTitleDelete: "Hapus Permission",
            textDataCreateSuccess: "Data Permission Berhasil ditambahkan.",
            textDataUpdateSuccess: "Data Permission Berhasil diupdate.",
    ...
    created(){
		this.$FUNCTIONS.checkPermissions(this.$dataRole['.cms.role.get'])
    },
    mounted(){
        this.getPost()
    ...
    methods:{
        ...
        buildPermissionListUrl(value){
			if(value){
				return this.$baseUrl + this.sectionApiListPermission
			}else{
				return this.$baseUrlAuthCMS + this.sectionApiListPermission
			}
        },
        async getPost(){
            try{
        ...
        async getPostPermissionList(){
            try{
                this.loadingPermission = true
                const response = await this.$services.getList(this.buildPermissionListUrl(true))
                this.loadingPermission = false
                if(response.status == 200){
					this.itemsDataPermission = response.data
					this.getPermissionListAuth()
                    // this.datasItems = {}
                    // let datas = []
                    // this.itemsDataPermission.forEach(function(e){
                    //     datas.push(e.split('.')[1])
                    // }, this)
                    // this.itemDataPermissionCards = this.$FUNCTIONS.removeDups(datas)
                    // this.itemDataPermissionCards.forEach(function(e){
                    //     let datas2 = []
                    //     this.itemsDataPermission.forEach(function(w){
                    //         if(w.search(e) !== -1){
                    //             datas2.push(w)
                    //         }
                    //     }, this)
                    //     this.datasItems[e] = datas2
                    // }, this)
                }else{
                    this.$FUNCTIONS.checkErrorResponse(response.status)
                }
		...
		async getPermissionListAuth(){
			try{
				this.loadingPermission = true
                const response = await this.$services.getList(this.buildPermissionListUrl(false))
				this.loadingPermission = false
				if(response.status == 200){
					this.itemsDataPermissionAuth = this.itemsDataPermission.concat(response.data)
				}
			}catch(err){
				console.log("error")
			}
            ...
            if(type == "post"){
                ...
                try{
                    ...
                    if(response.status == 200){
						...
						if(this.itemDataDetail.permissions !== null){
							this.forms.permissions = JSON.parse(this.itemDataDetail.permissions)
						}
		...
		handleCheckAllChange(val){
			this.forms.permissions = val ? this.itemsDataPermission : []
        	this.isIndeterminate = false
		},
		handleCheckedCitiesChange(value){
			let checkedCount = value.length;
			this.checkAll = checkedCount === this.itemsDataPermission.length;
			this.isIndeterminate = checkedCount > 0 && checkedCount < this.itemsDataPermission.length;
		}
    }
}
</script>
